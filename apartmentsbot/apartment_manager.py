...
class ApartmentManager:
    ...
    def load_apartments(self, city: str, area: float, max_price: int, rooms: int) -> None:
        self.apartments = self.execute_filter(city, area, max_price, rooms)
        self.index = 0

    def get_next_batch(self, batch_size=DEFAULT_BATCH_SIZE) -> list[Apartment]:
        start = self.index
        end = self.index = start + batch_size
        if end > len(self.apartments):
            end = len(self.apartments)
